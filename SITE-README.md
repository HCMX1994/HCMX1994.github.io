...
The active visual theme is **Modern Collegiate**: a deep teal navigation bar, a pale warm-grey academic-journey strip with official university logos, serif headings, a portrait on the right, and three research columns on desktop. The journey summarises four universities: Lancaster, Imperial, Kent and Xidian. Xidian includes both bachelor's and master's study; the complete experience timeline retains both Lancaster appointments, including the postdoctoral role. Local SVG and PNG identity assets are documented in `images/institutions/SOURCES.md`. The final `.collegiate-site` rules in `assets/css/academic-home.css` apply consistently to the homepage and the complete archive/detail pages. All content, the embedded BD-RIS lab, Scholar refresh and MapMyVisitors integration are retained; older news and projects remain expandable.

## Publish

The homepage photo is `images/xuekang-liu-airport.jpg`, retaining the supplied original's full 5273 × 3515 resolution with light JPEG compression (quality 98). Its 4:3 landscape crop is defined only by `.hero-portrait-window` and the image positioning in the stylesheet; no face retouching is applied. The desktop profile panel is capped at 437 px wide. Keep the full-resolution original separately when changing the crop later.
