...
第一份是 **2026 年 9 月的未结束月份快照**，仅保存截至 24 September 2026 14:26 UTC 的 13 位月度访客、13 次访问、17 次浏览、12 个国家/地区及 12 个城市。这些数据包含建站与 VPN 测试访问。9 月结束后需重新导出完整 9 月数据覆盖此快照。所有统计只使用 Umami，从 24 September 2026 开始；不导入任何旧统计服务的数据。
...
- **唯一统计来源**：主页、世界地图和历史档案只使用 Umami。已移除 MapMyVisitors 嵌入，不累计其旧数据。

## 主页展示

主页显示所有已归档月份的累计访问次数，另列当月访客数和国家/地区数量。地图标记位于国家/地区的参考坐标，**不代表访客的精确位置或城市坐标**；点击标记或选择国家后查看城市分布，城市占比也使用全部归档访问次数作分母。未定位地区保留在总数和地区列表中。

`scripts/visitor_panel.py` 在每次建站时读取同一份 `files/visitor-history.json`，因此导入后重新发布即可同步更新主页数字和地图。没有第三方地图 API 或额外统计服务；地图与交互脚本保存在网站本身。保留了时间戳、归档月份和手动更新提示。当月尚未导入时显示“—”，不会错误显示上个月的访客数。
