...
(function () {
  ...
  const state={example:'compare',field:'near',incidence:45,range:5,target:0,goal:'sidelobes',strength:.5,operation:'reflection',reflected:-25,transmitted:30,share:.5,playing:!motion.matches};
  ...
  function antennas(parent,transmission=false) {
    ...
    for(let i=0;i<M.N;i++) {
      const y=101+i*13;
      if(transmission) {element('line',{x1:228,y1:y,x2:252,y2:y,class:'bd-v3-link'},parent);element('rect',{x:248,y:y-4,width:5,height:8,class:'bd-v3-rear'},parent);}
      element('rect',{x:transmission?227:237,y:y-4,width:6,height:8,class:'bd-v3-port'},parent);
    }
  }
  ...
  function updateCompare() {
    ...
    const trans=state.operation==='transmission',side=trans?1:-1,g=find('.bd-grid');g.replaceChildren();grid(g,side);
    antennas(find('.bd-antenna-drawing'),trans);
    ...
    const name=trans?'Diagonal T':'D-RIS';all('.bd-d-name').forEach(n=>n.textContent=name);
    find('.bd-mode-label').textContent=trans?'Transmission':'Reflection';
    find('.bd-network-label').textContent=trans?'BD: 16 ports · 8 front + 8 rear':'BD: fully connected · 8 ports · group size 8';
    ...
    find('.bd-brief').textContent=state.goal==='peak'?(Math.abs(b.target-d.target)<1e-7?'Equal target power here: phase alignment already reaches the common bound.':b.target/d.target<1.01?'Nearly equal target power: the input amplitudes differ only slightly. The small BD improvement is shown in the details.':'BD-RIS redistributes unequal input amplitudes to increase power at the target.'):(b.meanSide<d.meanSide-1e-7?`BD-RIS lowers mean sidelobe-region power${targetTradeoff}`:'Compare the same target-power / sidelobe-region trade-off; extra freedom does not improve every metric.');
    find('.bd-comparison-condition').textContent=trans?'Reference = diagonal transmission block T, not a diagonal full S. Both designs use eight front and eight rear ports, with zero controlled reflection.':'Both designs use the same eight reflecting ports. D-RIS has eight independent one-port loads; BD-RIS uses one fully connected group of eight ports (group size = 8), with a symmetric unitary R.';
    ...
    find('.bd-model-health').textContent='Reciprocity, lossless power conservation and S-to-output consistency checked for this state.';
    matrixDisplay(matrixD,comparison.phase.S);matrixDisplay(matrixB,comparison.joint.S);
    find('.bd-d-matrix-title').textContent=trans?'Diagonal T · rear ← front':'D-RIS · R';find('.bd-b-matrix-title').textContent=trans?'BD-RIS T · rear ← front':'BD-RIS · R';
    find('.bd-matrix-values').textContent=`${trans?'16 ports, ordered [front 1…8, rear 1…8]':'8 reflecting ports'}\n\n${name}:\n${formatMatrix(comparison.S_d)}\n\nBD-RIS:\n${formatMatrix(comparison.S_b)}`;
    find('.bd-compare-plot').setAttribute('aria-label',`${state.field}-field illumination; ${state.operation}; target ${state.target} degrees. Dashed ${name} and solid BD-RIS use a common power scale and objective.`);
  }
  ...
  function draw(){if(state.example==='compare'&&comparison){const side=state.operation==='transmission'?1:-1;dot('.bd-flow-b',comparison.bMetrics.plot,side,240);dot('.bd-flow-d',comparison.dMetrics.plot,side,240,.4);}else if(pair){dot('.bd-flow-r',pair.rPattern,-1,226);dot('.bd-flow-t',pair.tPattern,1,254,.4);}}
  ...
  for(const key of ['operation','goal'])find(`#bd-${key}`).addEventListener('change',event=>{state[key]=event.target.value;update();});
